...
package com.aspirephile.taskcompanion.util.kankan.widget;
...
import com.aspirephile.taskcompanion.util.kankan.widget.adapters.WheelViewAdapter;
...
public class WheelView extends View {
    private Logger l = new Logger(WheelView.class);
    ...
    public void setViewAdapter(WheelViewAdapter viewAdapter) {
        if (asserter.assertPointer(this.viewAdapter)) {
    ...
    public void invalidateWheel(boolean clearCaches) {
        if (clearCaches) {
            recycle.clearAll();
            if (asserter.assertPointer(itemsLayout)) {
                itemsLayout.removeAllViews();
            }
            scrollingOffset = 0;
        } else if (asserter.assertPointer(itemsLayout)) {
            // cache all items
    ...
    private int getDesiredHeight(LinearLayout layout) {
        if (asserter.assertPointer(layout) && asserter.assertPointer(layout.getChildAt(0))) {
    ...
    private int getItemHeight() {
        ...
        if (asserter.assertPointer(itemsLayout) && asserter.assertPointer(itemsLayout.getChildAt(0))) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (asserter.assertPointer(viewAdapter) && viewAdapter.getItemsCount() > 0) {
            updateView();
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                if (asserter.assertPointer(getParent())) {
    ...
    private boolean addViewItem(int index, boolean first) {
        ...
        if (asserter.assertPointer(view)) {
            if (first) {
    ...
    private boolean isValidItemIndex(int index) {
        return asserter.assertPointerQuietly(viewAdapter)
    ...
    public void setCenterDrawable(int id) {
        centerDrawable = getResources().getDrawable(id);
    }
}
