...
public class WheelRecycle {
    private Logger l = new Logger(WheelRecycle.class);
    ...
    public void clearAll() {
        if (asserter.assertPointer(items)) {
            items.clear();
        }
        if (asserter.assertPointer(emptyItems)) {
            emptyItems.clear();
        }
    }
    ...
    private View getCachedView(List<View> cache) {
        if (asserter.assertPointerQuietly(cache) && cache.size() > 0) {
